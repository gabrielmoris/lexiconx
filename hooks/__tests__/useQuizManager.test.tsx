...
const mockWord1 = {
 _id: "word1", userId: "u1", word: "gato", definition: "cat",
 phoneticNotation: "gato", language: "Espa\u00f1ol", tags: [],
 lastReviewed: null, nextReview: new Date().toISOString(),
 interval: 0, repetitions: 0, easeFactor: 2.5,
};

const mockWord2 = {
 _id: "word2", userId: "u1", word: "perro", definition: "dog",
 phoneticNotation: "pero", language: "Espa\u00f1ol", tags: [],
 lastReviewed: null, nextReview: new Date().toISOString(),
 interval: 0, repetitions: 0, easeFactor: 2.5,
};
...
const mockQuiz = {
 ...
 options: [{ answer: "cat", isCorrect: false }, { answer: "dog", isCorrect: true }],
 usedWords: ["word2"],
 },
 ],
};
...
const mockUserData = {
 email: "test@test.com", name: "Test User", activeLanguage: "Espa\u00f1ol",
 learningProgress: [{
 language: "Espa\u00f1ol", level: 5, wordsMastered: 0,
 currentStreak: 3, lastSessionDate: new Date(), timeSpent: 0,
 }],
...
// --- Setup helper (always uses real timers) ---
...
describe("useQuizManager", () => {
 ...
 act(() => { result.current.handleAnswerClick(correctOption); });
 act(() => { vi.advanceTimersByTime(600); });
 act(() => { result.current.handleAnswerClick({ answer: "dog", isCorrect: true }); });
 act(() => { vi.advanceTimersByTime(600); });
 };

 const completeQuizFail = (result: any) => {
 act(() => { result.current.handleAnswerClick(wrongOption); });
 act(() => { vi.advanceTimersByTime(600); });
 act(() => { result.current.handleAnswerClick(wrongOption); });
 act(() => { vi.advanceTimersByTime(600); });
 };
