...
describe('generateQuizWithWords - input validation', () => {
  ...
  it('throws if level is below 1', async () => {
    await expect(generateQuizWithWords(words, 0, 'English', 'Español')).rejects.toThrow(
      'Level must be between 1 and 100'
    );
  });

  it('throws if level is above 100', async () => {
    await expect(generateQuizWithWords(words, 101, 'English', 'Español')).rejects.toThrow(
      'Level must be between 1 and 100'
    );
  });

  it('throws if empty words array', async () => {
    await expect(generateQuizWithWords([], 10, 'English', 'Español')).rejects.toThrow(
      'At least 3 words are required'
    );
  });
...
describe('generateQuizWithWords - response validation', () => {
  ...
  it('throws if response missing quizzes array', async () => {
    ...
    await expect(generateQuizWithWords(words, 10, 'English', 'Español')).rejects.toThrow(
      'Response missing quizzes array'
    );
  });
  ...
  it('throws if quiz has fewer than 3 questions', async () => {
    ...
    await expect(generateQuizWithWords(words, 10, 'English', 'Español')).rejects.toThrow(
      'must have between 3 and 5 questions'
    );
  });
  ...
  it('throws if question has no usedWords array', async () => {
    ...
    await expect(generateQuizWithWords(words, 10, 'English', 'Español')).rejects.toThrow(
      "must have a non-empty 'usedWords' array"
    );
  });
  ...
  it('throws if question has no correct answer', async () => {
    const response = makeValidQuizResponse();
    response.quizzes[0].questions[0].options.forEach(
