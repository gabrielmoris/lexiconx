...
EXPLANATION FIELDS - REQUIRED FOR EACH QUESTION:
- "elaboration": A 1-2 sentence explanation of why the correct answer carries its meaning in this context (etymology, word roots, cultural usage, or conceptual link). This helps the learner deepen encoding after a correct answer.
- "errorExplanation": A 1-2 sentence explanation of why the incorrect options are wrong in this context (e.g., false cognate, wrong register, wrong collocation). This helps the learner learn from errors after an incorrect answer.
Both fields are REQUIRED for every question.
...
"elaboration": "1-2 sentence explanation of why the correct answer carries its meaning in this context (etymology, word roots, cultural usage, or conceptual link)",
"errorExplanation": "1-2 sentence explanation of why the incorrect options are wrong in this context (e.g., false cognate, wrong register, wrong collocation)"
}
...
✓ Answer options language matches the ANSWER OPTIONS LANGUAGE RULES for level ${level}/100
      ✓ Each question has elaboration and errorExplanation fields
...
CAMPOS DE EXPLICACIÓN - OBLIGATORIOS PARA CADA PREGUNTA:
- "elaboration": 1-2 oraciones explicando por qué la respuesta correcta tiene su significado en este contexto (etimología, raíces, uso cultural o enlace conceptual). Esto ayuda al estudiante a profundizar la codificación tras una respuesta correcta.
- "errorExplanation": 1-2 oraciones explicando por qué las opciones incorrectas son erróneas en este contexto (p. ej., falso cognado, registro incorrecto, colocación errónea). Esto ayuda al estudiante a aprender de los errores tras una respuesta incorrecta.
Ambos campos son OBLIGATORIOS para cada pregunta.
...
"elaboration": "1-2 oraciones explicando por qué la respuesta correcta tiene su significado en este contexto (etimología, raíces, uso cultural o enlace conceptual)",
"errorExplanation": "1-2 oraciones explicando por qué las opciones incorrectas son erróneas en este contexto (p. ej., falso cognado, registro incorrecto, colocación errónea)"
}
...
ERKLÄRUNGSFELDER - ERFORDERLICH FÜR JEDE FRAGE:
- "elaboration": 1-2 Sätze Erklärung, warum die richtige Antwort in diesem Kontext ihre Bedeutung trägt (Etymologie, Wortwurzeln, kultureller Gebrauch oder konzeptionelle Verbindung). Dies hilft dem Lernenden, die Enkodierung nach einer richtigen Antwort zu vertiefen.
- "errorExplanation": 1-2 Sätze Erklärung, warum die falschen Optionen in diesem Kontext falsch sind (z.B. falscher Kognat, falsches Register, falsche Kollokation). Dies hilft dem Lernenden, aus Fehlern nach einer falschen Antwort zu lernen.
Beide Felder sind für jede Frage ERFORDERLICH.
...
"elaboration": "1-2 Sätze Erklärung, warum die richtige Antwort in diesem Kontext ihre Bedeutung trägt (Etymologie, Wortwurzeln, kultureller Gebrauch oder konzeptionelle Verbindung)",
"errorExplanation": "1-2 Sätze Erklärung, warum die falschen Optionen in diesem Kontext falsch sind (z.B. falscher Kognat, falsches Register, falsche Kollokation)"
}
...
解释字段 - 每个问题必须包含：
- "elaboration": 1-2句话解释正确答案为什么在这个语境中具有其含义（词源、词根、文化用法或概念联系）。这有助于学习者在答对后加深编码。
- "errorExplanation": 1-2句话解释错误选项为什么在这个语境中是错误的（例如：假同源词、语域错误、搭配错误）。这有助于学习者在答错后从错误中学习。
每个问题都必须包含这两个字段。
...
"elaboration": "1-2句话解释正确答案为什么在这个语境中具有其含义（词源、词根、文化用法或概念联系）",
"errorExplanation": "1-2句话解释错误选项为什么在这个语境中是错误的（例如：假同源词、语域错误、搭配错误）"
}
              // ... 更多问题 (每个测验项目总共 3-5 个)
            ]
          }
...
ПОЛЯ ОБЪЯСНЕНИЙ - ОБЯЗАТЕЛЬНЫ ДЛЯ КАЖДОГО ВОПРОСА:
- "elaboration": 1-2 предложения объяснения, почему правильный ответ несёт свой смысл в этом контексте (этимология, корни, культурное употребление или концептуальная связь). Это помогает учащемуся углубить кодировку после правильного ответа.
- "errorExplanation": 1-2 предложения объяснения, почему неправильные варианты ошибочны в этом контексте (напр., ложный когнат, неверный регистр, неверная коллокация). Это помогает учащемуся учиться на ошибках после неправильного ответа.
Оба поля ОБЯЗАТЕЛЬНЫ для каждого вопроса.
...
"elaboration": "1-2 предложения объяснения, почему правильный ответ несёт свой смысл в этом контексте (этимология, корни, культурное употребление или концептуальная связь)",
"errorExplanation": "1-2 предложения объяснения, почему неправильные варианты ошибочны в этом контексте (напр., ложный когнат, неверный регистр, неверная коллокация)"
}
